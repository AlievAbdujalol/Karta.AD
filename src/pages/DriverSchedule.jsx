    ...
    <div className="bg-white rounded-2xl p-5 shadow-sm space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-indigo-50 rounded-xl p-3 text-center">
          ...
          <p className="text-[11px] text-indigo-400 mt-0.5">Начало смены</p>
        </div>
        <div className="bg-purple-50 rounded-xl p-3 text-center">
          <p className="text-lg font-bold text-purple-700">{shiftEnd}</p>
          <p className="text-[11px] text-purple-400 mt-0.5">Конец смены</p>
        </div>
      </div>
      <div>
        <div className="flex items-center justify-between text-xs text-gray-500 mb-1.5">
          <span>Прогресс смены</span>
          <span className="font-semibold text-gray-700">{completedTrips} / {totalTrips} рейсов</span>
        </div>
  ...
  if (!activeStops.length) {
    return (
      <div className="bg-white rounded-2xl p-4 shadow-sm text-center text-gray-400 text-sm">
        ...
        Все остановки на сегодня выполнены!
      </div>
    );
  }
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-xs font-semibold truncate ${isNext ? 'text-green-800' : 'text-gray-700'}`}>
                  {stop.stop_name || `Остановка ${stop.index + 1}`}
                </p>
                {isNext && (
                  <p className="text-[10px] text-green-600 font-medium">Следующая остановка</p>
                )}
              </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm space-y-3">
            ...
            return (
              <div key={i} className="flex items-center gap-3 py-1.5 relative">
                ...
                <div className="flex-1">
                  <p className="text-xs text-gray-700">{stop.name || `Остановка ${i + 1}`}</p>
                </div>
                {(isFirst) && (
                  <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full font-semibold">Старт</span>
                )}
                {i === route.stops.length - 1 && (
                  <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-semibold">Финиш</span>
                )}
              </div>
            );
          })}
      ...
      {route.stops.length > 5 && (
        ...
        >
          {expanded ? 'Свернуть' : `Показать все ${route.stops.length} остановок`}
...
export default function DriverSchedule() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const vehicles = await base44.entities.Vehicle.filter({ driver_id: u.id, is_active: true });
      const v = vehicles[0] || null;
      setVehicle(v);

      if (v?.route_id) {
        const routes = await base44.entities.Route.filter({ id: v.route_id });
        const r = routes[0] || null;
        setRoute(r);
        if (r) {
          const schedules = await base44.entities.Schedule.filter({ route_id: r.id });
  ...
  if (user.role !== 'driver' || user.driver_status !== 'approved') {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center gap-4">
        ...
        <p className="text-gray-600 font-medium">Эта страница доступна только для подтверждённых водителей</p>
      </div>
    );
  }
      ...
      <div className="max-w-md mx-auto space-y-4 pb-6">
        ...
        <div className="bg-gradient-to-br from-blue-700 to-blue-900 rounded-2xl p-5 text-white">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white/10 rounded-xl px-3 py-2 text-center">
              <p className="text-xs text-blue-200">Транспорт №</p>
              <p className="font-bold text-white">{vehicle?.vehicle_number || '—'}</p>
            </div>
            <div className="bg-white/10 rounded-xl px-3 py-2 text-center">
              <p className="text-xs text-blue-200">Маршрут</p>
        ...
        {!vehicle && (
          <div className="bg-white rounded-2xl p-5 shadow-sm text-center text-gray-400 text-sm space-y-2">
            ...
            <p>Нет активной поездки. Начните смену в разделе «Водитель».</p>
          </div>
        )}
        ...
        {route && !schedule && (
          <div className="bg-white rounded-2xl p-4 shadow-sm text-center text-gray-400 text-sm">
            ...
            Расписание для этого маршрута ещё не задано администратором
          </div>
        )}
      </div>
    </div>
