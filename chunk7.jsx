  ...
  const buildRoute = useCallback(async () => {
    ...
    setFallbackWalk(null);
    setSelectedTransitIdx(0);
    setShowSteps(false);

    const isTransit = transportMode === 'bus' || transportMode === 'minibus';

    if (isTransit) {
      // Transit routing via findTransitRoutes
      const typeFilter = transportMode;
      let result;
      try {
        result = await findTransitRoutes(from, to, routes, typeFilter);
      } catch {
        result = { direct: [], transfers: [], best: null };
      }
  ...
  // Auto-build when both points are set, and on mode change
  const prevRef = useRef({ from: null, to: null, mode: null });
  useEffect(() => {
    const prev = prevRef.current;
    const modeChanged = prev.mode !== transportMode;
    const pointsChanged = prev.from !== from || prev.to !== to;
    prevRef.current = { from, to, mode: transportMode };
    if (from && to && (pointsChanged || modeChanged)) {
      buildRoute();
    }
  ...
  const handleStartNavigation = useCallback(() => {
    ...
    transportMode, selectedTransitOption, osrmRoute, fallbackWalk,
    from, to, fromText, toText, onStartNavigation, navCtl, onClose,
  ]);
  ...
  const handleShare = useCallback(async () => {
    ...
    const url = `${window.location.origin}${window.location.pathname}#route=${encodeURIComponent(JSON.stringify(payload))}`;
    try {
  ...
  const activeMode = TRANSPORT_MODES.find((m) => m.id === transportMode);
  const isTransit = transportMode === 'bus' || transportMode === 'minibus';
  ...
  const etaInfo = boardStopForEta && selectedTransitOption?.route
    ? etaToStop(vehicles, selectedTransitOption.route.id, boardStopForEta)
    : null;

  const arrivalTime = useMemo(() => {
    const dur = selectedTransitOption?.totalDuration || osrmRoute?.duration || fallbackWalk?.duration || 0;
    return new Date(Date.now() + dur * 1000).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' });
  }, [selectedTransitOption, osrmRoute, fallbackWalk]);
